...
class CountryData
  ...
  def self.highest_gni_per_capita_in_asia
    result = DatabaseConnection.query("SELECT * FROM countries
      WHERE gni = (SELECT MAX(gni) FROM countries WHERE continent = 'Asia');")
      result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
    end

  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries
      WHERE gni = (SELECT gni FROM countries WHERE continent = 'South America' ORDER BY gni limit 1 offset 2) ;")
      result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.countries_not_in_asia_hide_gni_per_capita_and_population_density
    result = DatabaseConnection.query("SELECT * FROM countries;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['continent'] == 'Asia' ? country['density'] : '', country['continent'] == 'Asia' ? country['gni'] : '') }
  end
end
